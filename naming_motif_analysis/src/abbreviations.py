...
class AbbreviationExpander:
    ...
    def load_file(self, filename):
        if filename.endswith(".xml") or filename.endswith(".xml.gz"):
            ...
            logging.warning(
                'Abbreviation file does not end in xml or tsv, ignoring: "{}"'.format(
                    filename
                )
            )

    def load_tsv(self, filename):
        logging.debug("Loading abbreviations from TSV file " + filename)
        count = 0
        ...
        for line in file:
            ...
            try:
                ...
                logging.error('Abbreviation line malformed: "{}"'.format(line))
        file.close()
        logging.debug("Loaded " + str(count) + " abbreviations")

    def load_biocxml(self, filename):
        logging.debug("Loading abbreviations from BioC file " + filename)
        count = 0
        ...
        for document in collection.documents:
            for passage in document.passages:
                ...
                for annotation in passage.annotations:
                    if (
                        not "type" in annotation.infons
                        or annotation.infons["type"] != "ABBR"
                    ):
                        continue
                    annotation_id2text[annotation.id] = annotation.text
                for relation in passage.relations:
                    if (
                        not "type" in relation.infons
                        or relation.infons["type"] != "ABBR"
                    ):
                        continue
                    long = None
                    short = None
                    ...
                    if not long is None and not short is None:
                        ...
                        logging.warning(
                            "Could not identify long form & short form for document "
                            + document.id
                            + " relation "
                            + relation.id
                        )
    ...
    def add(self, document_ID, short, long):
        ...
        regex = re.compile(rf"\b{re.escape(short)}\b")
        if regex.search(long):
            logging.debug(
                'INFO Ignoring abbreviation "'
                + short
                + '" -> "'
                + long
                + '" because long form contains short form'
            )
    ...
    def do_sub(self, short, long, text):
        if text.find(long) >= 0:
            return re.sub(rf"\s*\(\s*{re.escape(short)}\s*\)\s*", " ", text)
        # Change all non-overlapping instances of short to long
        updated = ""
        index = 0
        for match in re.finditer(rf"\b{re.escape(short)}\b", text):
